...
## DISTRIBUTION POLICY
Source is MIT and public. **Official notarized DMGs are sold on Gumroad only** — never attach binaries to GitHub Releases or publish them via public CI artifacts. Customer updates = new Gumroad file versions (re-download); no `electron-updater` / public feed yet. `packages/app/electron-builder.yml` sets `publish: null` to block accidental uploads. Placeholder product URL and full ship checklist: **`docs/DISTRIBUTION.md`**. Phase 0 play + bundled-ffmpeg gate: **passed 2026-07-24** (`docs/phase0-1.0.0-hardware.md`); scrub remains a known limitation.
...
- BUG #1 and BUG #2 both closed. **653 tests green across 51 files** (there are no CLI test files; re-counted 2026-07-24 — the long-published "502 across 44" was stale). NOTE: the root `npx tsc -b` does NOT cover `packages/app` (its tsconfig is deliberately outside the project-reference graph) — run `npm run typecheck -w packages/app` as well.
...
- **The bundle has never been cast from.** It launches and runs the real engine; no TV has seen it. Per the hardware protocol, only a human watching the screen settles that.
- **ffmpeg independence is UNPROVEN, and this is the sharpest trap in this section.** Every check above passes on this machine *even if bundling silently failed*, because Homebrew ffmpeg is still sitting at `/opt/homebrew/bin` as a working fallback. The real test is launching the installed app with `/opt/homebrew/bin/ffmpeg` and `ffprobe` **temporarily renamed** — anything short of that is measuring the wrong machine.
- **The DMG's own notarization ticket may not be stapled** (above). The `.app` inside it is.
- The DMG is **not installed**; `/Applications/` still holds the 2026-07-23 ad-hoc `castgorilla.app`, so anything tested there is the previous build. Expect a fresh TCC prompt on install (predicted, unverified); user data does NOT move (verified).
...
cd packages/app && APPLE_KEYCHAIN_PROFILE=castgorilla npm run dist:dmg
...
DMG="release/Cast Gorilla-1.0.0-arm64.dmg"
...
```
`spctl` and `stapler validate` both fall back to an ONLINE lookup — green here does not prove a stapled ticket. Install: mount the DMG and drag to `/Applications` (or `rm -rf "/Applications/Cast Gorilla.app" && cp -R "packages/app/release/mac-arm64/Cast Gorilla.app" /Applications/`).
...
## Next milestones
0. ~~Verify the 1.0.0 release bundle~~ **DONE 2026-07-24** for play + bundled ffmpeg — see `docs/phase0-1.0.0-hardware.md`. Remaining: optional offline DMG staple check; Gumroad upload + merge/tag.
